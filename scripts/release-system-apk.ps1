...
#   .\scripts\release-system-apk.ps1 -VersionName 0.1.1
#
# Version lives in app\version.properties (VERSION_NAME + VERSION_CODE).
# Output: install\out\geely-ex2-range-v{VERSION_NAME}({VERSION_CODE}).apk
# Also copies to install\out\geely-ex2-range-system-platform-signed.apk
...
if (-not $props.ContainsKey("VERSION_NAME")) { $props["VERSION_NAME"] = "0.1.0" }
...
$code = [int]$props["VERSION_CODE"]
$apkFileName = "geely-ex2-range-v${name}($code).apk"
...
$signedStable = Join-Path $outDir "geely-ex2-range-system-platform-signed.apk"
...
Write-Host "    versionName=$name  versionCode=$code  (Android shows $name-system ($code))"
Write-Host "Also: $signedStable"
Write-Host ""
Write-Host "Install:"
Write-Host "  adb uninstall com.geely.ex2.range"
Write-Host "  adb push `"$signedNamed`" /data/local/tmp/geely-ex2-range-system.apk"
Write-Host "  adb shell pm install -r -g /data/local/tmp/geely-ex2-range-system.apk"
